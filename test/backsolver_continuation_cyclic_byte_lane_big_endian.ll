...
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); c=next(x for x in r['memory_slots'][0]['exit_states'] if x['state_kind']=='cyclic-byte-composition'); assert r['analysis']=='llvm-memoryssa-aa-cyclic-byte-lane-revalidated-v6' and c['endianness']=='big' and [x['source_kind'] for x in c['backedge_state']['lanes']]==['carry','store']"

target datalayout = "E-m:e-i64:64-n32:64"
target triple = "powerpc64-unknown-linux-gnu"

define i16 @choose_loop_big(i8 %selector, i8 %iterations, ptr %slot) {
entry:
...
; BIG: cyclic-byte-lane-continuation-memory-tuple-v6
