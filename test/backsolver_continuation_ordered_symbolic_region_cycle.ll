...
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); s=r['memory_slots'][0]; c=next(x for x in s['exit_states'] if x['state_kind']=='ordered-symbolic-region-cyclic-byte-composition'); b=c['backedge_state']; w=b['symbolic_region_writers']; assert r['analysis']=='llvm-memoryssa-aa-ordered-symbolic-region-cyclic-byte-lane-revalidated-v19' and s['state_schema']=='ordered-symbolic-region-cyclic-byte-lane-continuation-memory-tuple-v19' and [x['ordinal'] for x in w]==[0,1,2] and len({x['store_site'] for x in w})==3 and len({x['region_base_site'] for x in w})==1 and {x['region_extent'] for x in w}=={'8'} and [x['source_kind'] for x in b['lanes']]==['carry']*4 and all([[y['index_value'] for y in z['cases']] for z in x['lane_cases']]==[[0],[1],[2],[3]] for x in w)"
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); w=next(x for x in r['memory_slots'][0]['exit_states'] if x['state_kind']=='ordered-symbolic-region-cyclic-byte-composition')['backedge_state']['symbolic_region_writers']; w[0],w[1]=w[1],w[0]; w[0]['ordinal']=0; w[1]['ordinal']=1; open(r'%t.order-tamper','w').write(json.dumps(r)+'\n')"
; RUN: not %python %S/../util/verify_ifss_continuation_manifest.py %t.order-tamper
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); w=next(x for x in r['memory_slots'][0]['exit_states'] if x['state_kind']=='ordered-symbolic-region-cyclic-byte-composition')['backedge_state']['symbolic_region_writers']; w[1]['region_extent']='9'; open(r'%t.extent-tamper','w').write(json.dumps(r)+'\n')"
...
; RUN: %python -c "p=open(r'%t.lowered.ll').read(); old='icmp eq i64 %new.index, 2'; new='icmp eq i64 %new.index, 7'; assert p.count(old)==1; open(r'%t.value-tamper.ll','w').write(p.replace(old,new,1))"
...
; RUN: %python -c "import pathlib,subprocess; lli=str(pathlib.Path(r'%opt').with_name('lli')); vals=((0,0,0),(17,34,51),(255,1,2),(1,255,3),(170,85,204)); run=lambda p,a,n,x,y,z: subprocess.run([lli,'--entry-function=semantic_main',p],input=bytes([a,n,x,y,z]),stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode; assert all(run(r'%t.original.ll',a,n,x,y,z)==run(r'%t.lowered.ll',a,n,x,y,z) for a in (64,65,66) for n in range(7) for x,y,z in vals)"
...
; RUN: %python -c "import os,subprocess; e=dict(os.environ,SYMCC_OUTPUT_DIR=r'%t-out',SYMCC_AFL_COVERAGE_MAP=r'%t-map',SYMCC_TELEMETRY_OUT=r'%t.json'); subprocess.run([r'%t'],input=b'C\\x03\\x11\\x22\\x33',env=e,check=True)"
...
; RUN: %python -c "import glob; models=[open(p,'rb').read() for p in glob.glob(r'%t-out/*-backsolve')]; assert models and any(len(m)>=5 and m[0]==65 for m in models)"
; RUN: %python %S/../util/cross_llvm_transform_replay.py verify --certificate %S/../benchmark/evidence/ifss_f296_cross_llvm_certificate.json

target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-pc-linux-gnu"

define i8 @choose_loop(
    i8 %selector, i8 %iterations, i8 %old.value,
    i8 %middle.value, i8 %new.value) {
entry:
...
body:
  ...
  store i8 %old.value, ptr %old.element, align 1
  %middle.element = getelementptr inbounds i8, ptr %slot, i64 %index
  store i8 %middle.value, ptr %middle.element, align 1
  %new.index = add i64 %index, 1
  %new.element = getelementptr inbounds i8, ptr %slot, i64 %new.index
  store i8 %new.value, ptr %new.element, align 1
...
entry:
  ...
  %read = call i64 @read(i32 0, ptr %input, i64 5)
  %selector.ptr = getelementptr inbounds [5 x i8], ptr %input, i64 0, i64 0
  %iterations.ptr = getelementptr inbounds [5 x i8], ptr %input, i64 0, i64 1
  %old.ptr = getelementptr inbounds [5 x i8], ptr %input, i64 0, i64 2
  %middle.ptr = getelementptr inbounds [5 x i8], ptr %input, i64 0, i64 3
  %new.ptr = getelementptr inbounds [5 x i8], ptr %input, i64 0, i64 4
  %selector = load i8, ptr %selector.ptr, align 1
  %iterations = load i8, ptr %iterations.ptr, align 1
  %old.value = load i8, ptr %old.ptr, align 1
  %middle.value = load i8, ptr %middle.ptr, align 1
  %new.value = load i8, ptr %new.ptr, align 1
...
; LOWERED: %ifss.cont.byte.region = select i1 %ifss.cont.region.hit, i8 %old.value
; LOWERED: %ifss.cont.byte.region{{[0-9]*}} = select i1 {{.*}}, i8 %middle.value
; LOWERED: %ifss.cont.byte.region{{[0-9]*}} = select i1 {{.*}}, i8 %new.value
; LOWERED: ordered-symbolic-region-cyclic-byte-lane-continuation-memory-tuple-v19
...
; IFSS: ordered-symbolic-region-cyclic-byte-lane-continuation-memory-tuple-v19
