...
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); s=r['memory_slots'][0]; p=next(x['pointer_partition'] for x in s['exit_states'] if x['state_kind']=='finite-pointer-union'); assert r['analysis']=='llvm-memoryssa-aa-finite-pointer-union-revalidated-v7' and s['state_schema']=='finite-pointer-union-continuation-memory-tuple-v7' and len(p['nodes'])==3 and len(p['leaves'])==4 and [x['lane_source_bytes'] for x in p['leaves']]==[[-1,0],[-1,-1],[-1,-1],[-1,0]]"
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); p=next(x['pointer_partition'] for x in r['memory_slots'][0]['exit_states'] if x['state_kind']=='finite-pointer-union'); p['nodes'][0]['true_index']=2; open(r'%t.tree-tamper','w').write(json.dumps(r)+'\n')"
; RUN: not %python %S/../util/verify_ifss_continuation_manifest.py %t.tree-tamper
...
; RUN: %python -c "p=open(r'%t.lowered.ll').read(); old='select i1 %guard1, i8 51, i8 %ifss.cont.byte'; new='select i1 %guard1, i8 52, i8 %ifss.cont.byte'; assert p.count(old)==1; open(r'%t.value-tamper.ll','w').write(p.replace(old,new,1))"
...
; RUN: %python -c "import pathlib,subprocess; lli=str(pathlib.Path(r'%opt').with_name('lli')); run=lambda p,a,g0,g1: subprocess.run([lli,'--entry-function=semantic_main',p],input=bytes([a,g0,g1]),stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode; assert all(run(r'%t.original.ll',a,g0,g1)==run(r'%t.lowered.ll',a,g0,g1) for a in range(256) for g0 in range(2) for g1 in range(2))"
...
; RUN: %python -c "import glob; models=[open(p,'rb').read() for p in glob.glob(r'%t-out/*-backsolve')]; assert models and any(len(m)>=3 and m[:1]==b'A' and ((m[1]==0)==(m[2]==0)) for m in models)"

target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128"
target triple = "x86_64-pc-linux-gnu"

define i8 @choose_union(
    i8 %selector, i8 %key0, i8 %key1, ptr %slot) {
entry:
...
entry:
  ...
  %key0.ptr = getelementptr inbounds [3 x i8], ptr %input, i64 0, i64 1
  %key1.ptr = getelementptr inbounds [3 x i8], ptr %input, i64 0, i64 2
  %selector = load i8, ptr %selector.ptr, align 1
  %key0 = load i8, ptr %key0.ptr, align 1
  %key1 = load i8, ptr %key1.ptr, align 1
  %choice = call i8 @choose_union(
      i8 %selector, i8 %key0, i8 %key1, ptr %slot)
...
; LOWERED-LABEL: define i8 @choose_union
; LOWERED: %ifss.cont.byte.partition = select i1 %guard1
; LOWERED: %ifss.cont.byte.partition{{[0-9]+}} = select i1 %guard1
; LOWERED: %ifss.cont.byte.partition{{[0-9]+}} = select i1 %guard0
; LOWERED: %ifss.cont.memory = phi i16
; LOWERED: finite-pointer-union-continuation-memory-tuple-v7

; IFSS-LABEL: define i8 @choose_union
; IFSS: %ifss.cont.byte.partition = select i1 %guard1
; IFSS: %ifss.cont.memory = phi i16
; IFSS: call ptr @_sym_build_ite
; IFSS: finite-pointer-union-continuation-memory-tuple-v7
