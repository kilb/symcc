; REQUIRES: qsym
; RUN: %symcc -O0 -S -emit-llvm %s -o - | FileCheck %s --check-prefix=CHAIN
; RUN: %symcc -O0 %s -o %t
...
right:
  ...
  store i8 2, i8* %right_noise, align 1
  br label %merge
...
; CHAIN: %ifss.memory.state = call ptr @_sym_build_ite
; CHAIN-SAME: !symcc.ifss_memory
; CHAIN: ![[PROOF:[0-9]+]] = !{!"must-alias-memoryssa-chain-v1",
; CHAIN-SAME: i32 1, i64
; CHAIN-SAME: i32 1, i64
