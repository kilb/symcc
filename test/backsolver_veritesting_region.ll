...
; RUN: python3 -c "import glob; models=[open(p,'rb').read() for p in glob.glob(r'%t-out/*-backsolve')]; assert models and any(m[0:1] != b'A' and m[1:2] == b'X' and m[2:3] == b'Y' for m in models)"
...
left:
  %left_value = add i8 %second, 0
  br label %merge

right:
  %right_value = add i8 %third, 1
  br label %merge
