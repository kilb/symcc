...
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); s=r['memory_slots'][0]; c=next(x for x in s['exit_states'] if x['state_kind']=='conditional-multi-latch-cyclic-byte-composition'); b=c['backedge_states']; assert r['analysis']=='llvm-memoryssa-aa-conditional-multi-latch-cyclic-byte-lane-revalidated-v12' and s['state_schema']=='conditional-multi-latch-cyclic-byte-lane-continuation-memory-tuple-v12' and [x['transfer_kind'] for x in b]==['conditional','unconditional'] and [[y['source_kind'] for y in x['lanes']] for x in b]==[['guarded-store','carry'],['carry','store']] and b[0]['cycle_store_when']=='true'"
; RUN: %python -c "import json; r=json.loads(open(r'%t.manifest').read()); c=next(x for x in r['memory_slots'][0]['exit_states'] if x['state_kind']=='conditional-multi-latch-cyclic-byte-composition'); c['backedge_states'][0]['cycle_store_when']='false'; open(r'%t.polarity-tamper','w').write(json.dumps(r)+'\n')"
...
; RUN: %python -c "import os,subprocess; e=dict(os.environ,SYMCC_OUTPUT_DIR=r'%t-out',SYMCC_AFL_COVERAGE_MAP=r'%t-map',SYMCC_TELEMETRY_OUT=r'%t.json'); subprocess.run([r'%t'],input=b'C\\x02\\x01',env=e,check=True)"
...
; RUN: %python -c "import glob; models=[open(p,'rb').read() for p in glob.glob(r'%t-out/*-backsolve')]; assert models and any(len(m)>=3 and m[:1]==b'A' and m[1]>=2 and m[2]!=0 for m in models)"
...
body:
  ...
  br i1 %use.low, label %low.branch, label %latch.high
...
; LOWERED-LABEL: loop:
; LOWERED: %ifss.cont.memory.multi.cycle = phi i16
; LOWERED-LABEL: latch.low:
; LOWERED: %ifss.cont.byte.cycle.guard = select i1 %write.guard, i8 51
; LOWERED-LABEL: latch.high:
; LOWERED: %ifss.cont.byte.extend{{[0-9]*}} = zext i8 68 to i16
; LOWERED: conditional-multi-latch-cyclic-byte-lane-continuation-memory-tuple-v12

; IFSS-LABEL: define i8 @choose_loop
; IFSS: %ifss.cont.memory.multi.cycle = phi i16
; IFSS: %ifss.cont.byte.cycle.guard = select i1 %write.guard
; IFSS: %ifss.cont.memory = phi i16
; IFSS: call ptr @_sym_build_ite
; IFSS: conditional-multi-latch-cyclic-byte-lane-continuation-memory-tuple-v12
