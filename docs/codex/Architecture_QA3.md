...
>
> **配图分两类，互不替代**：
> **① 25 张示意图与统计图**（`docs/codex/diagrams/qa3/`，SVG + PNG + DOT 图源）——解释**机制**：
> 执行流、进程拓扑、位图全景、工作项时序、过滤漏斗、约束形态分流、求解决策树、
> 丢前缀粒度、嵌套深度、种子长度、ICSE'23 再评估与 benchmark 数据等。
> **② 7 张一手证据截图**（`docs/codex/evidence/`，[说明](evidence/README.md)）——给出**当场可见的原始输出**：
...
**【一手证据】编译产物层面**（`docs/codex/evidence/img_d2_pcguard.png`）：对一个 4 个 `if` 的小目标做
...
**【一手证据】**（`docs/codex/evidence/img_showmap.png`）：pcre2 上跑这条命令，输出同时印证了三件事——
...
> **F307 当前语义补充（2026-08-06）**：下方步骤中的 showmap 逻辑已经进一步加固。
> `StreamingShowmap` 的实现迁到 `util/afl_streaming_showmap.py`，streaming `-S` 只接受
> stdin target command；含 `@@` 的 target 会被拒绝，因为本机 AFL++ 4.40c 的 `-S`/`-I`
> 不能安全替换该占位符。QA3 新增 `--input-mode auto|stdin|file`：项目 AFL persistent/SHM
> 标记按约定走 stdin，通用 target 应显式指定；file 模式只走 one-shot
> `afl-showmap -- target absolute_input_path`。terminal status 也不再被折叠成空覆盖：
> `normal-only` 下 fail closed，`stratified` 下单独计数但不给 coverage credit。完整说明和
> 当前 evidence 见
...
4. 首个 item 时惰性建 `StreamingShowmap`，最多重试 5 次（`:4757-4777`）。失败则退化为 master 侧全量 showmap。F307之后，只有 stdin ABI 可使用 streaming；file/placeholder ABI 必须走状态可见的 one-shot。
...
12. **批量 showmap**（`:1194-1204`），退化则逐个流式 `get_result`/one-shot。F307之后，batch `-I` 只保留精确stdin target命令；placeholder target 不再尝试自动拼 `@@`。
13. **B2 合并判新**（`:1207-1231`），只把状态正常、边集非空、且经status-preserving路径验证的候选塞进 `new_tests` ——相位 **`showmap_dedup`**。
...
**【一手证据】③ AFL 自己的计数器**（`docs/codex/evidence/img_fuzzer_stats.png`）：pcre2 跑了 9 秒后的 `fuzzer_stats` 里
...
**【一手证据】④ 这条结论会怎样影响既有报告**（`docs/codex/evidence/img_process.png`）：一次 **60 秒**的
...
> **2026-07-31 实测修订**：上述 `suffixes.add()` 判断是审查误报。lit 的
> `TestingConfig` setter 会把 `lit.cfg` 赋入的 list 规范化为 set，因此 site config
> 加载时 `.add(".test32")` 正确；隔离的 `TARGET_32BIT=ON` 配置成功递归发现 231 项测试。
> 真正修复的是干净环境 PATH：site config 现在把 CMake 已知的 LLVM tools 目录前置，
> 使直接调用 `FileCheck`/`llc` 的历史测试不再依赖调用者 shell。完整证据见
> [`Correctness_Hardening_2026-07-31.md`](Correctness_Hardening_2026-07-31.md#10-lit-配置审查修订)。
...
**【一手证据】① 编译产物**（`docs/codex/evidence/img_ir.png`）：`build/symcc -S -emit-llvm` 后的 IR 里可直接看到
...
**【一手证据】② 运行时**（`docs/codex/evidence/img_gdb.png`）：`gdb -ex 'b _sym_push_path_constraint' -ex run` 断下来时，
...
5. CoFuzz 用边级在线回归调度 + 采样增强同步，边覆盖 +16.31 %（15/15 夺冠，
   13 个 p<0.01），共定位 42 个 bug，其中 37 个此前未知、30 个获确认、20 个已修复，
   8 个获得 CVE；这些是论文报告值，不是本项目复现实验结果。
...
> **2026-07-31 指标修订**：`generated` 是兼容字段，hybrid 为 AFL executions 与
> SymCC candidates 的混合 work-unit，AFL-only 为 `execs_done`，MPI/serial 为 SymCC
> candidates，不能跨 kind 直接比较。新 schema 同时持久化 `generated_kind`、
> `throughput_kind`、`afl_executions`、`symcc_generated`、retained file 数和内容 hash
> 并集；文本报告只对相同 throughput kind 计算 speedup。详见
...
**【一手证据】**（`docs/codex/evidence/img_report.png`）：下面这张表的**一手来源**就是
...
16. Huang, Yao, Wu, Shi, Zhang. *Pangolin: Incremental Hybrid Fuzzing with Polyhedral
    Path Abstraction*, **IEEE S&P 2020**．
    [作者公开 PDF](https://home.cse.ust.hk/~charlesz/papers/pangolin.pdf)｜
    [DOI](https://doi.org/10.1109/SP40000.2020.00063)
    —— polyhedral path abstraction、受约束采样和增量复用的原始方法；当前项目只实现
    有界子集，不能用“Pangolin 式”替代完整复现声明。
17. Lu, Li, Viswanathan, Lattner. *Taming the Hydra: Targeted Control-Flow
    Transformations for Dynamic Symbolic Execution*, **OOPSLA 2026**．
    [会议页面](https://2026.splashcon.org/details/oopsla-2026/3/Taming-the-Hydra-Targeted-Control-Flow-Transformations-for-Dynamic-Symbolic-Executio)｜
    [DOI](https://doi.org/10.1145/3798202)
    —— failure-preserving 而非完整 semantics-preserving 的变换边界。

**工具文档 / 源码**

18. AFL++ `instrumentation/afl-compiler-rt.o.c` —— §1.4 的 PCGUARD 边 ID 分配（`__sanitizer_cov_trace_pc_guard` / `__afl_final_loc`）：
   <https://github.com/AFLplusplus/AFLplusplus/blob/stable/instrumentation/afl-compiler-rt.o.c>
19. AFL++ `docs/env_variables.md` —— §1.10 的 `AFL_SYNC_TIME` 默认值（20 分钟，`-M` 减半）：
