...
python3 benchmark/analyze_current_evaluation.py \
  --group legacy=docs/codex/evidence/current-eval-2026-07-30/paired_15s_legacy.csv \
  --group current=docs/codex/evidence/current-eval-2026-07-30/paired_15s_current.csv \
...
## Post-report compiler semantic replay

F297 replays the same libarchive 7zip object compile after adding scalar LLVM
integer min/max symbolization. The compile succeeds, the ten former
`smin`/`umin`/`umax` concretization warnings are absent, and the remaining
single diagnostic is the unchanged source bit-field warning. See
[`libarchive_7zip_object_after_f297.log`](libarchive_7zip_object_after_f297.log).
This is a mechanism-level real-target check, not a coverage-performance claim.

F298 replaces direct writes into the AFL master's own queue with AFL++ native
campaign-peer synchronization from `symcc01/queue`. The 75 s mechanism run
records 23 published, 22 scanned by the native cursor, four retained imports,
and one explicitly reported hard-stop tail. All four imported files match their
SymCC source SHA-256. See
[`f298_native_peer_sync_2026-07-31.md`](f298_native_peer_sync_2026-07-31.md).
This proves the online transfer and attribution contract, not a general
