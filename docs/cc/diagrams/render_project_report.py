#!/usr/bin/env python3
"""渲染 docs/cc/Project_Report_2026-07.md 的配图（SVG + PNG，可复现）。

本文件与 docs/cc/ 下的产物由本汇报材料独占维护；配色与图元从
docs/diagrams/render_qa3_diagrams.py 引入，只读不改。

配色、字体与 render_qa3_diagrams.py 完全一致，两套图集混排时视觉统一。
结构图走 Graphviz DOT，定量图直接生成 SVG。所有数值均来自：
  - docs/evidence/current-eval-2026-07-30/independent_comparisons.csv（20 轮独立样本比较）
  - docs/evidence/lava_m_current_2026_07_30/base64_summary.json（LAVA-M 求解消融）
  - docs/symsan_hybrid_benchmark_6targets.md（SymSan 六目标 3 轮均值）
  - docs/Architecture_QA3.md（漏斗、策略占比、LAVA-M、持久模式实测）
...
HERE = Path(__file__).resolve().parent          # docs/cc/diagrams
REPO = HERE.parent.parent.parent                 # 仓库根
sys.path.insert(0, str(REPO / "docs" / "diagrams"))  # 复用 QA3 图集的配色与图元

from render_qa3_diagrams import (  # noqa: E402
    BLUE, BLUE_BG, DOT_HEADER, FONT, GREEN, GREEN_BG, INK, LINE, MUTED,
    ORANGE, ORANGE_BG, PANEL, RED, RED_BG, TEAL, TEAL_BG, VIOLET, VIOLET_BG,
    dot_source, esc, line, rect, svg_document, text,
)
...
GRAPHS = {
  ...
  subgraph cluster_rt {
    label="② 运行时　runtime/src　6.5k 行（链进目标进程）"; fontsize=14;
...
def chart_headline_forest() -> None:
    ...
    for i, (grp, tgt, med, cl, ch, a12, p, sig) in enumerate(rows):
        ...
        parts.append(text(px(ch) + 14, y + 12, f"{med:+.3f}", size=15, weight=700,
                          fill=col))
        # 区间只画成方框、读者无法核对具体数值；标在框下左对齐，避开右侧 A12 列
        parts.append(text(px(cl), y + 34, f"[{cl:+.3f}, {ch:+.3f}]",
...
def chart_scale() -> None:
    ...
    archive = (repo / "docs/New_Implementation_Archive.md").read_text(encoding="utf-8")
    feats = len(set(re.findall(r"\bF[0-9]{2,3}\b", archive)))
    test_files = [f for f in (repo / "test").iterdir() if f.is_file()]
    by_ext: dict[str, int] = {}
    for f in test_files:
    ...
    cards = [
        ...
        (f"{py / 1000:.0f}k", "行 Python（util/）", [f"{py_files} 个模块",
         f"runtime/src 另有 {rt / 1000:.1f}k 行 C++"], RED, RED_BG),
        (f"{docs / 1000:.0f}k", "行文档（docs/）", ["含权威档案、SOTA 审计", "与逐项证据清单"], INK, PANEL),
...
def chart_evidence_grades() -> None:
    ...
    tiers = [
        ...
        ("B 级　多轮描述", "多轮均值 + 全距，未做等 CPU 与随机化",
         ["SymSan 六目标 3 轮均值 + 全距", "（§4.3）"], TEAL, TEAL_BG, 800),
        ("C 级　机制描述", "单次或不同 seed 的有界测量，用于解释机制与量级，不提供重复运行推断",
         ["LAVA 求解技术族消融（§4.4）· 漏斗 / 策略占比", "求解开关 / 种子长度 / 持久模式 / 扩展性（§4.5–4.11）"], ORANGE, ORANGE_BG, 920),
...
def chart_sota() -> None:
    ...
    rows = [
        ...
        ("Pangolin", "IEEE S&P’20", "polyhedral context、增量复用、多解采样", "跨前缀 / 跨布局路径不传播 UNSAT"),
...
def chart_lava_ablation() -> None:
    ...
    parts.append(rect(48, 672, 1504, 160, fill=PANEL, stroke=LINE, radius=10))
    parts.append(text(
        72, 704,
...
def normalize_svg_width(path: pathlib.Path, lo: int = 1000, hi: int = 1500) -> None:
