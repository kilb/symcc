...
| **规模** | 仓库历史 578 次提交，其中 P01–P07 追踪 139 个项目定制提交；当前未提交工作树覆盖 P08–P10 · 297 个功能条目 F00–F296 · `test/` 一级目录 231 个文件 · `Configuration.txt` 记录 375 个去重 `SYMCC_*` 名称 |
...
| **② 运行时** | `runtime/src/` | 6.5k 行 | 链进目标进程，构造表达式与路径约束 | `RuntimeCommon.cpp`、`Shadow.cpp`、`LibcWrappers.cpp`、`UCSanRuntime.cpp`；后端 `backends/qsym`（Z3 + 分支剪枝图 B3）与 `backends/simple`（SMT-LIB 转储，供 lit 断言） |
...
| **⑤ 质量与文档** | `test/`、`docs/` | `test/` 一级目录 231 个文件<br>44.1k 行文档 | 验证接口与不变量；记录方案 / 证据 / 局限 | `test/*.ll`(102)、`*.c`(54)、`*.py`(52)、`*.test32`(17)；`New_Implementation_Archive.md` |
...
[`Configuration.txt`](../Configuration.txt) 当前记录 **375 个去重 `SYMCC_*` 名称**
...
telemetry 证明这些技术确实被执行了，而不是"只配置未运行"：
...
| `poly_cross_prefix_probes` | 5,169 |
...
更多 listed bug。
因此 [§6.1](#61-明确不能宣称的结论) 第 1 条禁语**没有解除**。它只是从"完全无法陈述"
变成"可以陈述一组必须成对出现的事实"：完整路径 Z3 调用 −56.6 %、候选 +33.7 %，
**同时** solver queries +190.5 %、候选吞吐 −97.5 %、墙钟 ×53.3、bug 数持平。
**只报前半句就是误导。**
...
| 目标 | 配置 | ShowmapCov | FstatsCov | AFL 执行数 | SymCC 产出 / interesting |
|---|---|---:|---:|---:|---:|
| SQLite | current Hybrid | **28.04 %**（8,848/31,552） | 26.16 % | 63,604,671 | 102 / 不可恢复* |
| SQLite | current AFL-only | 27.46 % | **27.59 %** | 14,929,395 | 0 / 0 |
| libarchive | current Hybrid | **24.31 %**（3,330/13,696） | **23.69 %** | 81,561,081 | 169 / 不可恢复* |
| libarchive | current AFL-only | 20.85 % | 20.87 % | 21,731,887 | 0 / 0 |

> \* **`interesting` 一列为什么标"不可恢复"**：这份 300 s CSV 早于 `symcc_interesting`
> 的解析修复，原始文件里 SQLite 的 47、libarchive 的 50 是**运行中的早期进度快照**，
> 不是最终累计值。修复后的口径已在 10 s 冒烟测试中验证。原始 CSV 有意保持不动以留痕，
> 但这两个数**不得作为结果引用**。
...
上面所有实验都跑在一棵**通过了完整验证的工作树**上。验证记录如下
（源自 [`evidence/current-eval-2026-07-30/verification.md`](../evidence/current-eval-2026-07-30/verification.md)）：

| 检查项 | 结果 |
|---|---|
| `cmake --build build -j16` | exit 0 |
| `py_compile` 三个关键脚本 | exit 0 |
| **完整 lit 测试套件** | **207 个用例发现，207 个全部通过**，135.22 s |
| 定向 lit（当前分析 + memory-state 拒绝） | 2 passed |
| `git diff --check` | exit 0 |
| 统计图 SVG 结构解析 + headless Chrome 栅格化 | 有效，1120 × 815，人工目检 |
| SQLite / libarchive SymCC 目标冒烟运行 | 均 exit 0 |
| 20 轮 legacy / current campaign | 164 行 CSV（含 4 行 seed），**全部 `success`，目标崩溃数 0** |
| 300 s current campaign | 6 行 CSV（含 2 行 seed），全部 `success`，崩溃数 0 |
| 修复后 10 s telemetry 冒烟 | `generated=4434`、`afl_generated=4434`、`symcc_generated=0`、`symcc_interesting=0`，崩溃 0 |

两点需要说清：

1. **"207 个 lit 用例"与图 R-2 的"232 个 `test/` 一级文件"不是一回事**——后者是目录下的
   文件总数（含 `.inc` 夹具、`README` 等非用例文件），前者是 lit 实际发现并执行的用例数。
2. **"崩溃数 0"只说明本轮 campaign 没有触发崩溃**，既不能推出被测程序无漏洞，
   也不能推出系统没有找 bug 的能力（见 [§6.1](#61-明确不能宣称的结论)）。

> ⚠ 另一条来自证据目录 README 的纪律，本报告严格遵守：
> **`paired_comparisons.csv` 属于审计留痕，不得用于本报告的任何结论**——
> 因为 20 轮重复的 repeat index 并非随机区组、不具备设计上的配对语义。
> 现行分析把两批 campaign 当作**独立样本**处理，结论一律取自
> [`independent_comparisons.csv`](../evidence/current-eval-2026-07-30/independent_comparisons.csv)。
...
| **Pangolin**（S&P'20） | polyhedral context、增量复用、多解采样 | 跨前缀 / 跨布局路径**不传播 UNSAT** |
...
| **Hydra**（OOPSLA'26） | state merge、fork elision、failure-preserving 变换 | 默认关闭，必须 replay / manifest 验证 |
...
python3 docs/diagrams/render_qa3_diagrams.py       # QA3 图集 25 张
python3 docs/diagrams/render_project_report.py     # 本报告图集 21 张
```
