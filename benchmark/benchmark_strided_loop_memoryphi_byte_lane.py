#!/usr/bin/env python3
"""Reference-validator mechanism benchmark for F412."""
...
def benchmark(
    *, object_bytes: int = 64, stride: int = 8,
    ...
    case = StridedInductionCase(
        ...
        load_bytes, tuple(range(aliases)),
    )
    ...
    return {
        "schema": "symcc-strided-loop-memoryphi-benchmark-v1",
        ...
        "analytic_certificate_cardinality": {
            "writer_aliases": len(writer_aliases(case)),
            "reachable_writers": len(reachable_writers(case)),
            "covered_writer_bytes": (
                len(reachable_writers(case)) * writer_bytes
            ),
            "load_aliases": aliases,
